---
name: merp
description: >-
  Activates when the user asks about Medical Expense Reimbursement Plan applicability, recommendation,
  or projected tax savings. Uses savings-core runtime (marginal rate × strategy change).
metadata:
  status: outlined
  fidelity: none
  spe_folder: "Medical Expense Reimbursement Plan"
  ita_id: "ita_016"
  category: "business"
  outline: ita-rules/strategy-outlines/merp.md
  config: ita-rules/strategy_runtime/configs/merp.json
  recommended_tools:
    - name: assess_merp_applicability
      description: Returns whether strategy is applicable/recommended (strategy_change > 0).
    - name: estimate_merp_savings
      description: Returns projected tax savings and cash outlay (savings-core formula).
---

# Medical Expense Reimbursement Plan

**Status:** implemented (savings-core fidelity).

Deterministic tools delegate to [`strategy_runtime`](../../../../ita-rules/strategy_runtime/engine.py).
Field definitions: [`ita-rules/strategy_runtime/configs/merp.json`](ita-rules/strategy_runtime/configs/merp.json).

See `INPUTS.md` and [`ita-rules/strategy-outlines/merp.md`](ita-rules/strategy-outlines/merp.md) for SPE context.

---

## Part 1 — Applicability

Call **`assess_merp_applicability`** with form fields from the config.
Applicable when `strategy_change > 0` after defaults/expression.
Default `strategy_change` = `merp_reimbursement`.

---

## Part 2 — Savings

Call **`estimate_merp_savings`** when applicable.

```
projected_tax_savings = round(strategy_change × marginal_rate_total / 100)
cash_outlay = strategy_change - projected_tax_savings  (contribution-like strategies)
```

Mode: `marginal_x_change` · Cash outlay: `contribution`

---

## Form fields

- `merp_reimbursement` (advisor)
- `strategy_change` (advisor)
- `federal_marginal_rate_pct` (engine)
- `state_marginal_rate_pct` (engine)
- `nyc_marginal_rate_pct` (engine)

---

## Protocol

```
Payload (engine + user + advisor fields)
        │
        ▼
assess_merp_applicability
        │
        ├── not applicable → done
        │
        ▼
estimate_merp_savings
        │
        ▼
Show savings / cash outlay
```
